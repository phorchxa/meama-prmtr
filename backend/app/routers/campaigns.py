...
async def list_campaigns(sb=Depends(get_supabase)) -> list[CampaignSummary]:
    ...
    try:
        res = sb.rpc("execute_readonly_query", {"query_text": _CAMPAIGNS_QUERY}).execute()
        rows = res.data or []
    except Exception as exc:
        logger.warning("meta_insights unavailable, falling back: %s", exc)
        try:
            res = sb.rpc("execute_readonly_query", {"query_text": _CAMPAIGNS_NO_META_QUERY}).execute()
            rows = res.data or []
...
def _detail_query(cid: str, *, with_meta: bool) -> str:
    ...
    SELECT
      ...
      p.type AS promo_type, p.discount_value, p.shopify_code,
      p.discount_type, p.min_order_value, p.valid_from, p.valid_to, p.tag_pattern, p.excluded_segments,
      cr.revenue_total, cr.roi, cr.converted, cr.reached, cr.conversion_rate, cr.avg_order_value,
      {meta_select}
    FROM campaigns.campaigns c
