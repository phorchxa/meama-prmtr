-- ============================================================
-- 0015 · Fix SKU mismatch between order_items and products_master
--
-- Problem: meama_georgia_order_items.sku stores the long Shopify
-- variant SKU (e.g. cap51-1208), but products_master.sku stores the
-- short internal code (e.g. cap51-08).  All product RPCs joined
-- directly on sku = sku, so mismatched products returned zero
-- revenue and zero units_sold.
--
-- Fix: in every items CTE, LEFT JOIN "SKU Matching" and coalesce
-- the order-item SKU to the products_master SKU before aggregating.
--   "unified Code"        = long Shopify variant SKU  (order_items)
--   "Product variant SKU" = short products_master SKU
-- ============================================================
...
items AS (
    SELECT COALESCE(sm."Product variant SKU", i.sku::TEXT) AS sku,
    ...
    LEFT JOIN "SKU Matching" sm ON sm."unified Code" = i.sku::TEXT
